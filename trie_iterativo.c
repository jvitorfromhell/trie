...
bool insere(Nodo * trie, char * palavra) {
    ...
    while (pos < strlen(palavra) && atual->prox[palavra[pos] - OFFSET]) {
        atual = atual->prox[palavra[pos] - OFFSET];
        pos += 1;
    }
    ...
    while (pos < strlen(palavra)) {
        //printf("%d %c %d\n", pos, palavra[pos], strlen(palavra));
        atual->prox[palavra[pos] - OFFSET] = (Nodo *) malloc(sizeof(Nodo));
        atual = atual->prox[palavra[pos] - OFFSET];
        pos += 1;
    }
