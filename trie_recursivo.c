...
// Insere palavra na trie, efetivamente falando
...
bool insere_novo(Nodo * trie, char * palavra, int pos) {
    ...
    trie->prox[palavra[pos] - OFFSET] = (Nodo *) malloc(sizeof(Nodo));
    return insere_novo(trie->prox[palavra[pos] - OFFSET], palavra, pos+1);
}
...
bool insere(Nodo * trie, char * palavra, int pos) {
    ...
    // Ja existe palavra ateh o prefixo corrente
    if (trie->prox[palavra[pos] - OFFSET]) {
        return insere(trie->prox[palavra[pos] - OFFSET], palavra, pos + 1);
    }
    else {
    // Ja existe
        return insere_novo(trie, palavra, pos);     
    }
}
...
bool procura(Nodo * trie, char * palavra, int pos) {
    ...
    if (trie->prox[palavra[pos] - OFFSET]) {
        return procura(trie->prox[palavra[pos] - OFFSET], palavra, pos + 1);
    }
